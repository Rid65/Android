...
public class BaseActivity extends AppCompatActivity
    ...
    private static final int PERMISSION_CODE_CALL_PHONE = 1;
...
//        setContentView(linearLayout);
    ...
    private void initPermissions() {
        if (ActivityCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
            Toast.makeText(this, "Определение местоположения разрешено!", Toast.LENGTH_LONG).show();
        } else {
            requestPermitions(PERMISSION_CODE_LOCATION);
        }
    }
        ...
        switch (id) {
            ...
            case R.id.action_info:
                ...
                if (ActivityCompat.checkSelfPermission(this, Manifest.permission.CALL_PHONE) == PackageManager.PERMISSION_GRANTED) {
                    ...
                    requestPermitions(PERMISSION_CODE_CALL_PHONE);
                }
                return true;
    ...
    private void requestPermitions(int permitionCode) {
        if (permitionCode == PERMISSION_CODE_CALL_PHONE) {
            if (!ActivityCompat.shouldShowRequestPermissionRationale(this, Manifest.permission.CALL_PHONE)) {
                ActivityCompat.requestPermissions(this, new String[]
                        {Manifest.permission.CALL_PHONE}, PERMISSION_CODE_CALL_PHONE);
            }
        } else if (permitionCode == PERMISSION_CODE_LOCATION) {
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        if (requestCode == PERMISSION_CODE_CALL_PHONE) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                Intent intent = new Intent(Intent.ACTION_CALL, Uri.parse("tel:1234567"));
                startActivity(intent);
            }
        } else if (requestCode == PERMISSION_CODE_LOCATION) {
            Toast.makeText(this, "Получено разрешение на определение местоположения!", Toast.LENGTH_SHORT).show();
        }
    }
